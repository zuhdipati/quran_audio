...
class MockAmbientBloc extends MockBloc<AmbientEvent, AmbientState>
    implements AmbientBloc {}
...
void main() {
  ...
  late MockAmbientBloc mockAmbientBloc;

  setUpAll(() {
    registerFallbackValue(FakePlayerEvent());
    registerFallbackValue(FakePlayerState());
    registerFallbackValue(AmbientSoundsRequested());
  });

  setUp(() {
    mockPlayerBloc = MockPlayerBloc();
    mockAmbientBloc = MockAmbientBloc();
    when(() => mockAmbientBloc.state).thenReturn(const AmbientState());
  });
  ...
  group('Audio Player Page', () {
    testWidgets('should fire LoadSurah on init and display basic UI', (
      tester,
    ) async {
    ...
    testWidgets(
      'should display pause icon when playing and trigger PauseAudio on tap',
      (tester) async {
        when(() => mockPlayerBloc.state).thenReturn(
          const PlayerState(
    ...
    testWidgets(
      'should display play icon when paused and trigger ResumeAudio on tap',
      (tester) async {
        when(() => mockPlayerBloc.state).thenReturn(
          const PlayerState(
    ...
    testWidgets('should trigger PlayAudio when status is initial on tap', (
      tester,
    ) async {
      when(() => mockPlayerBloc.state).thenReturn(
        const PlayerState(
    ...
    testWidgets(
      'should trigger NextSurah when next icon is tapped and has next surah',
      (tester) async {
        when(() => mockPlayerBloc.state).thenReturn(
          const PlayerState(
            status: PlayerStatus.playing,
    ...
    testWidgets(
      'should trigger PreviousSurah when previous icon is tapped and has previous surah',
      (tester) async {
        when(() => mockPlayerBloc.state).thenReturn(
          const PlayerState(
            status: PlayerStatus.playing,
    ...
    testWidgets('should trigger SeekAudio for rewind 10 seconds', (
      tester,
    ) async {
      when(() => mockPlayerBloc.state).thenReturn(
        const PlayerState(
      ...
      // current 30s - 10s = 20s
      verify(
        () => mockPlayerBloc.add(const SeekAudio(Duration(seconds: 20))),
      ).called(1);
    });

    testWidgets('should trigger SeekAudio for fast forward 10 seconds', (
      tester,
    ) async {
      when(() => mockPlayerBloc.state).thenReturn(
        const PlayerState(
      ...
      // current 30s + 10s = 40s
      verify(
        () => mockPlayerBloc.add(const SeekAudio(Duration(seconds: 40))),
      ).called(1);
    });

    testWidgets('should not exceed duration when fast forwarding', (
      tester,
    ) async {
      when(() => mockPlayerBloc.state).thenReturn(
        const PlayerState(
      ...
      // capped at duration
      verify(
        () => mockPlayerBloc.add(const SeekAudio(Duration(seconds: 60))),
      ).called(1);
    });
    ...
    testWidgets('should not go below zero when rewinding', (tester) async {
      ...
      // floored at 0
      verify(
        () => mockPlayerBloc.add(const SeekAudio(Duration.zero)),
      ).called(1);
    });

    testWidgets(
      'should trigger StopAudio and pop when back button is pressed',
      (tester) async {
